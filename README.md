...
- 마지막 약 4초는 **띡·띡·띡** 간격이 점점 길어지는 감속 구간입니다.
- 상단의 고정 핀을 축으로 화살표가 각 칸 경계마다 튕기는 효과와 짧은 클릭음을 넣었습니다.
- 칸 이름은 중앙에서 바깥쪽으로 뻗는 방사형 배치입니다.
- 룰렛 중앙 원과 상단 핀은 각각 정확한 중앙 좌표에 고정했습니다.


## 이번 디자인 변경

- 상단 룰렛 제목을 화면에서 직접 입력할 수 있으며 브라우저에 자동 저장됩니다.
- 데스크톱 룰렛 크기를 최대 760px로 키웠습니다.
- 모든 룰렛 칸의 글자는 동일한 크기로 표시하고, 중앙에서 바깥 방향으로 배치합니다.
- 순위표는 색상 순위 배지와 고정 높이 스크롤 영역으로 정리했습니다.
